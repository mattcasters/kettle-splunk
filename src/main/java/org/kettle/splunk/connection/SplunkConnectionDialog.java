...
public class SplunkConnectionDialog {
  private static Class<?> PKG = SplunkConnectionDialog.class; // for i18n purposes, needed by Translator2!!

  private SplunkConnection SplunkConnection;
  ...
  public SplunkConnectionDialog( Shell parent, SplunkConnection SplunkConnection ) {
    this.parent = parent;
  ...
  public boolean open() {
    ...
    shell.setText( BaseMessages.getString( PKG, "SplunkConnectionDialog.Shell.Title" ) );
  ...
  private void addFormWidgets() {
    ...
    wHostname = new TextVar( SplunkConnection, shell, SWT.SINGLE | SWT.LEFT | SWT.BORDER );
    ...
    wPort = new TextVar( SplunkConnection, shell, SWT.SINGLE | SWT.LEFT | SWT.BORDER );
    props.setLook( wPort );
    ...
    wUsername = new TextVar( SplunkConnection, shell, SWT.SINGLE | SWT.LEFT | SWT.BORDER );
    ...
    wPassword = new PasswordTextVar( SplunkConnection, shell, SWT.SINGLE | SWT.LEFT | SWT.BORDER );
  ...
  public void getData() {
    wName.setText( Const.NVL( SplunkConnection.getName(), "" ) );
    wHostname.setText( Const.NVL( SplunkConnection.getHostname(), "" ) );
    wPort.setText( Const.NVL( SplunkConnection.getPort(), "" ) );
    wUsername.setText( Const.NVL( SplunkConnection.getUsername(), "" ) );
    wPassword.setText( Const.NVL( SplunkConnection.getPassword(), "" ) );
  ...
  public void ok() {
    if ( StringUtils.isEmpty( wName.getText() ) ) {
      ...
      box.setText( BaseMessages.getString( PKG, "SplunkConnectionDialog.NoNameDialog.Title" ) );
      box.setMessage( BaseMessages.getString( PKG, "SplunkConnectionDialog.NoNameDialog.Message" ) );
      box.open();
      return;
    }
    getInfo( SplunkConnection );
    ok = true;
    dispose();
  }

  // Get dialog info in securityService
  private void getInfo( SplunkConnection splunk ) {
  ...
  public void test() {
    SplunkConnection splunk = new SplunkConnection( SplunkConnection ); // parent as variable space
    try {
      getInfo( splunk );
      splunk.test();
      ...
      message += Const.CR;
      message += "Hostname : " + splunk.getRealHostname()+", port : "+splunk.getRealPort()+", user : "+splunk.getRealUsername();
      box.setMessage( message );
      box.open();
    } catch ( Exception e ) {
      new ErrorDialog( shell, "Error", "Error connecting to Splunk with Hostname '" + splunk.getRealHostname()+"', port "+splunk.getRealPort()+", and username '"+splunk.getRealUsername(), e );
    }
  }
}
